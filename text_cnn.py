    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        with tf.name_scope("evaluate_measures"):
            ...
            self.tp = tf.reduce_sum(tf.cast(tf.equal(p_prediction, p_label), "float"))
            ...
            self.tn = tf.reduce_sum(tf.cast(tf.equal(n_prediction, n_label), "int64"))
            self.fn = tf.subtract(tf.reduce_sum(self.predictions), self.tn)
            # cast tp, fp, tn, fn to float64
            self.tp = tf.cast(self.tp, tf.float64, name="tp")
            self.fp = tf.cast(self.fp, tf.float64, name="fp")
            self.tn = tf.cast(self.tn, tf.float64, name="tn")
            self.fn = tf.cast(self.fn, tf.float64, name="fn")
            # Precision
